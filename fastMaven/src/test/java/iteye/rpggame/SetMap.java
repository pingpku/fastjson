...
public class SetMap extends JFrame implements MapConfig{
    ...
    class PanelListenner extends MouseAdapter{
        public void mouseClicked(MouseEvent e) {
            ...
            //修改数组中的值
            if(Integer.parseInt(boxtype.getSelectedItem().toString())==1){
                map1[i][j] = num;
                icons1[i][j] = icon;
            }else if(Integer.parseInt(boxtype.getSelectedItem().toString())==2){
                map2[i][j] = num;
                icons2[i][j] = icon;
            }else if(Integer.parseInt(boxtype.getSelectedItem().toString())==3){
                map3[i][j] = num;
                icons3[i][j] = icon;
            }
            System.out.println(Integer.parseInt(boxtype.getSelectedItem().toString()));
